...
- discovers Argo CD cluster Secrets by the `argocd.argoproj.io/secret-type=cluster` label and indexes them by their `data.name` and `data.server`, so it resolves both legacy (v1) Secrets and v2 API-registered Secrets whose `metadata.name` is auto-generated by Argo CD
- finds matching Argo CD `Application` objects by **both** `spec.destination.name` (v1 and Akuity v2) and `spec.destination.server` (plain Argo CD v2), unioned and de-duplicated
...
### v1 vs v2 Argo CD Integration (Auto-Detected)

vCluster Platform has two ways of registering a tenant cluster with Argo CD, and the watcher supports both without any mode flag:
...
When no patchable cluster Secret can be resolved (for example a fully Akuity-hosted Argo CD where the Secret is not readable from this cluster), the watcher does **not** claim to pause. With the optional `ARGOCD_API_*` discovery configured, it stays limited to the wake/refresh signals. There is no per-cluster pause API in Argo CD: skip-reconcile lives only on the cluster Secret annotation.
...
| --- | --- | --- |
| `ARGOCD_CLUSTER_SECRET_NAME_TEMPLATES` | `loft-{project}-virtualcluster-{virtualcluster},loft-{project}-vcluster-{virtualcluster}` | Comma-separated, ordered list of expected cluster name templates. Auto-detects both the v2 (`virtualcluster` infix) and legacy v1 (`vcluster` infix) conventions. Supports `{project}` and `{virtualcluster}`. Each template is tried both bare and with the v2 `-argocd` suffix. Names are matched against Secret `data.name`; the truncated form is matched by prefix as a fallback |
| `ARGOCD_CLUSTER_SECRET_NAME_TEMPLATE` | none | Singular form, kept for backward compatibility. When set (and the plural is not), it pins a single template and behaves exactly as before |
...
| `ARGOCD_API_BASE` | disabled | Optional Argo CD REST API base URL (for example `https://argocd.example.com`). Only needed when cluster Secrets or Applications are not readable from this cluster (for example fully Akuity-hosted Argo CD). Enables discovery only; pause is never done over the API |
| `ARGOCD_API_TOKEN` | none | Bearer token for the Argo CD API. Required when `ARGOCD_API_BASE` is set unless `ARGOCD_API_TOKEN_PATH` is used |
| `ARGOCD_API_TOKEN_PATH` | none | Path to a file containing the Argo CD API bearer token |
| `ARGOCD_API_INSECURE` | `false` | When `true`, skips TLS verification for the Argo CD API client |
| `ARGOCD_API_CA_PATH` | none | CA bundle used to verify the Argo CD API endpoint when `ARGOCD_API_BASE` is `https://...` |
| `ARGOCD_API_TIMEOUT` | `10s` | Timeout for Argo CD API requests |
